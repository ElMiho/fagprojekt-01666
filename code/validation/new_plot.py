...
file_list = [file for file in os.listdir(folder_path) if file.startswith("TEDS") and file.endswith(".pkl")]
...
zi = zip(dis[0], dis[1], dis[2], dis[3], dis[4],dis[5],dis[6])
data = [row1 + row2 + row3 + row4+ row5 +row6 + row7  for row1, row2, row3, row4, row5, row6, row7 in zi]
